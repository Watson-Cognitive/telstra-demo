...
if(/loadSegments\=true/i.test(document.location.search)){CQ.shared.HTTP.get(CQ.shared.HTTP.externalize(a+".segment.js"))
...
c("Set segments to: "+v);
CQ_Analytics.ClientContext.set("aamsegments/segments",v)
}}function h(s,u){u=u||m;
if(r){if(!s.sid){u();
d.fireEvent("update")
}else{$.getJSON(r,{t:s.sid.join()},function(v){if(v.segments){k(v.segments)
}u();
d.fireEvent("update")
